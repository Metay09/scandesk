...
export default function DataPage({ fields, records, onDelete, onEdit, onExport, onImport, customers, settings, toast, isAdmin, currentShift, user }) {
  ...
  const THead = ({ showCust, rows: scopeRows }) => {
    ...
    return (
      <thead><tr>
        <th>#</th>
        <th style={{ width: 34 }}><input type="checkbox" checked={sel.size>0 && scope.length>0 && scope.every(r=>sel.has(r.id))} onChange={e => { if (e.target.checked) setSel(p => new Set([...p, ...scope.map(r=>r.id)])); else setSel(p => { const n=new Set(p); scope.forEach(r=>n.delete(r.id)); return n; }); }} /></th>{allF.map(f => <th key={f.id}>{f.label}</th>)}
